...
import { ObjectFieldCore } from "@/domains/ui/formv2/field";
 ...
 */
export function LLMProviderControllerCore(props: LLMProviderControllerCoreProps) {
...
export type LLMProviderControllerCore = ReturnType<typeof LLMProviderControllerCore>;
...
export function LLMProviderStore(props: LLMProviderStoreProps) {
  ...
  let _pendingProvider = { name: "" };
  let _pendingModel = { id: "" };
    ...
    get providers() {
      return _providers.map((provider) => {
        ...
        return {
          ...
          apiKey: value.apiKey,
          models: value.models.map((m) => ({ id: m.id, name: m.name, enabled: m.enabled, buildin: m.buildin })),
        };
      });
    },
  ...
  return {
    ...
    patch(
      ...
      for (let i = 0; i < ids.length; i += 1) {
        ...
        const buildinModels = payload.models.filter((m) => m.buildin);
        const customModels = payload.models.filter((m) => !m.buildin);
        value.models.forEach((m) => {
          const model = buildinModels.find((m) => m.id === m.id);
          if (model) {
            m.setEnabled(model.enabled);
            return;
          }
        });
        value.models.push(
    ...
    updateProviderApiProxyAddress(payload: { provider_id: string; apiProxyAddress: string }) {
      const value = _values[payload.provider_id];
      if (!value) {
        return;
      }
      value.updateApiProxyAddress(payload.apiProxyAddress);
      bus.emit(Events.ProviderChange, {
    ...
    toggleModelEnabled(payload: { provider_id: string; model_id: string; enabled: boolean }) {
      const value = _values[payload.provider_id];
      if (!value) {
        return;
      }
    ...
    updatePendingModel(payload: { id: string }) {
      _pendingModel = payload;
    },
    addPendingModel(payload: { provider_id: string; model_id?: string }) {
      ...
      value.models.push(
        LLMProviderModelValue({
          id: payload.model_id || _pendingModel.id,
          name: payload.model_id || _pendingModel.id,
          enabled: true,
          buildin: false,
        })
      );
    ...
    onProviderChange(handler: Handler<TheTypesOfEvents[Events.ProviderChange]>) {
